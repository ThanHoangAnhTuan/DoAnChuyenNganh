...
func (c *CFacilityDetail) CreateFacilityDetail(ctx *gin.Context) {
	...
	if err != nil {
		...
		global.Logger.Error("CreateFacilityDetail validation error: ", zap.String("error", err.Error()))
	...
	global.Logger.Info("CreateFacilityDetail success: ", zap.String("info", fmt.Sprintf("create facilityDetail success: %s", data.Id)))
...
func (c *CFacilityDetail) GetFacilityDetail(ctx *gin.Context) {
	...
	if err != nil {
		fmt.Printf("GetFacilityDetail error\n: %s", err.Error())
